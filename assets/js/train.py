...
def gen_input(img, skg, ini_texture, ini_mask,
              xcenter=64, ycenter=64, size=40):
...
def gen_input_rand(img, skg, seg, size_min=40, size_max=60, num_patch=1):
    ...
    for i in range(bs):
        ...
        for j in range(num_patch):
            ...
            if False:#len(seg_one) != 0:
                seg_select_index = int(rand_between(
                    0, seg_one.view(-1).size()[0]-1))
...
def gen_local_patch(patch_size, batch_size, eroded_seg, seg, img):
    ...
    for i_bs in range(bs):
        ...
        if len(seg_one) != 0:
            ...
            x, y = get_coor(
                seg_one[random_select],
                eroded_seg[i_bs, 0, :, :].size())
            # print x,y,i_bs
        else:
            x, y = (w/2, h/2)
        ...
        k = 1
        while torch.sum(seg[i_bs, 0, xstart:xend, ystart:yend]
...
def visualize_training(
            netG, val_loader, input_stack, target_img, target_texture, segment,
        vis, loss_graph, args):
    imgs = []
    for ii, data in enumerate(val_loader, 0):
        ...
        # torch.ones(seg[:,1,:,:].size())*(1-seg[:,1,:,:]).float()
        temp[:, 1, :, :] = 0
        # torch.ones(seg[:,2,:,:].size())*(1-seg[:,2,:,:]).float()
        temp[:, 2, :, :] = 0
       ...
       # import pdb; pdb.set_trace()
        ...
        texture_patch = gen_local_patch(
            patchsize, batch_size, eroded_seg, seg, outputlll)
        gt_texture_patch = gen_local_patch(
            patchsize, batch_size, eroded_seg, seg, targetlll)

    if args.color_space == 'lab':
        out_img = vis_image(
            custom_transforms.denormalize_lab(
                outputG.data.double().cpu()),
            args.color_space)
        temp_labout = custom_transforms.denormalize_lab(
            texture_patch.data.double().cpu())
        temp_labout[:, 1:3, :, :] = 0

        temp_labgt = custom_transforms.denormalize_lab(
            gt_texture_patch.data.double().cpu())
        temp_labgt[:, 1:3, :, :] = 0
        # torch.cat((patches[0].data.double().cpu(),patches[0].data.double().cpu(),patches[0].data.double().cpu()),1)
        temp_out = vis_image(temp_labout, args.color_space)
        #temp_out = (temp_out + 1 )/2

        # torch.cat((patches[1].data.double().cpu(),patches[1].data.double().cpu(),patches[1].data.double().cpu()),1)
        temp_gt = vis_image(temp_labgt, args.color_space)
        ...
        out_img = vis_image(
            custom_transforms.denormalize_rgb(
                outputG.data.double().cpu()),
            args.color_space)
    ...
    gt_patch = [x * 255 for x in temp_gt]
    for t_i in range(bs):
        #import pdb; pdb.set_trace()
        patchsize = int(args.local_texture_size)
        out_final[t_i][
            :, 0: patchsize, 0: patchsize] = out_patch[t_i][
            :, :, :]  # .append(np.resize(out_patch[t_i], (3,w,h)))
        # gt_final.append(np.resize(gt_patch[t_i], (3,w,h)))
        gt_final[t_i][:, 0:patchsize, 0:patchsize] = gt_patch[t_i][:, :, :]
    ...
    segment_img = [x * 255 for x in eroded_seg.cpu().numpy()]
    ...
    vis.line(
        np.array(
            loss_graph["gs"]), win='gs', opts=dict(
            title='G-Style Loss'))
    vis.line(
        np.array(
            loss_graph["g"]), win='g', opts=dict(
            title='G Total Loss'))
    vis.line(
        np.array(
            loss_graph["gd"]), win='gd', opts=dict(
            title='G-Discriminator Loss'))
    vis.line(
        np.array(
            loss_graph["gf"]), win='gf', opts=dict(
            title='G-Feature Loss'))
    vis.line(
        np.array(
            loss_graph["gpl"]), win='gpl', opts=dict(
            title='G-Pixel Loss-L'))
    vis.line(
        np.array(
            loss_graph["gpab"]), win='gpab', opts=dict(
            title='G-Pixel Loss-AB'))
    vis.line(np.array(loss_graph["d"]), win='d', opts=dict(title='D Loss'))
    if args.local_texture_size != -1:
        vis.line(
            np.array(
                loss_graph["dl"]), win='dl', opts=dict(
                title='D Local Loss'))
        vis.line(
            np.array(
                loss_graph["gdl"]), win='gdl', opts=dict(
                title='G D Local Loss'))


def train(model, train_loader, val_loader, input_stack, target_img,
    ...
    for i, data in enumerate(train_loader):
        ...
        #tic = time.time()
        if args.input_texture_patch == 'original_image':
            inp, _ = gen_input_rand(
                img, skg, eroded_seg[:, 0, :, :],
                args.patch_size_min, args.patch_size_max, args.
                num_input_texture_patch)
        elif args.input_texture_patch == 'dtd_texture':
            inp, _ = gen_input_rand(
                txt, skg, eroded_seg[:, 0, :, :],
                args.patch_size_min, args.patch_size_max, args.
                num_input_texture_patch)
        # print(time.time()-tic)
        ...
        err_pixel_ab = args.pixel_weight_ab * \
            criterion_pixel_ab(outputab, targetab)
        ...
        err_feat = args.feature_weight * \
            criterion_feat(out_feat, gt_feat.detach())
        ...
        err_pixel_l = args.global_pixel_weight_l * \
            criterion_pixel_l(outputl, targetl)
        if args.local_texture_size == -1:  # global, no loss patch
            ...
            for m in range(len(output_style_feat)):
                ...
                err_style += args.style_weight * \
                    criterion_style(gram_y, gram_s.detach())
            ...
            for p in range(args.num_local_texture_patch):
                ...
                texture_patchl = gen_local_patch(
                    patchsize, batch_size, eroded_seg, seg, outputl)
                gt_texture_patchl = gen_local_patch(
                    patchsize, batch_size, eroded_seg, seg, targetl)
                ...
                err_pixel_l += args.local_pixel_weight_l * \
                    criterion_pixel_l(texture_patchl, gt_texture_patchl)
                ...
                outputD_local = netD_local(
                    torch.cat((texture_patchl, gt_texture_patchl), 1))
        ...
        if args.local_texture_size != -1:
            ...
            clamped_output_D = torch.round(clamped_output_D)
            for acc_i in range(batch_size):
                score[acc_i] = torch.sum(
                    clamped_output_D[acc_i]) / D_output_size
            ...
            if args.color_space == 'lab':
                # outputD_local = netD_local(torch.cat((txtl[:, :, x1:(x1 +
                # patchsize), y1:(y1 + patchsize)],outputl[:, :, x2:(x2 +
                # patchsize), y2:(y2 + patchsize)]),1))#outputD =
                # netD(outputl.detach())
                outputD_local = netD_local(
                    torch.cat((texture_patchl, gt_texture_patchl), 1))
            elif args.color_space == 'rgb':
