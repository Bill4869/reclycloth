...
def texture_synth(texture, target, patch_length = 70):
    ...
    # patch_length the smaller the longer time it takes
    ite = 1 # the bigger the longer time it takes

    start_time = time.time()

    res = transferIter(texture, target, patch_length, ite)

    print('time: {}'.format(time.time() - start_time))

    res_cv = img_as_ubyte(res)
    res_cv_bgr = cv2.cvtColor(res, cv2.COLOR_RGB2BGR)

    return res_cv
    # cv2.imwrite("result_cv2.jpg", res_cv_bgr)
